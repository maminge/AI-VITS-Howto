...
右击我的电脑——属性——选上面的“高级”项——点击“性能”的“设置”——弹出“性能选项”，选“高级”——下面又“虚拟内存”项，点"设置"——弹出“虚拟内存”对话框 你的内存多大就设置比内存大一倍就行了。
```
```
3、报错：UnboundLocalError: local variable 'audio' referenced before assignment
答：上传的推理音频需要是16位整数wav格式，用Au转换一下就好。或者装个ffmpeg一劳永逸地解决问题。
```
```
4、报错：AssertionError: CPU training is not allowed.
答：非N卡跑不了的。
```
```
5、报错：torch.cuda.OutOfMemoryError: CUDA out of memory
答：爆显存了，试着把batch_size改小，改到1还爆的话建议云端训练。
```
```
6、报错：RuntimeError: DataLoader worker (pid(s) xxxx) exited unexpectedly
答：把虚拟内存再调大一点。
```
```
7、报错：NotImplementedError: Only 2D, 3D, 4D, 5D padding with non-constant padding are supported for now
答：数据集切片切太长了，5-10秒差不多。
```
```
8、报错：CUDA error: CUBLAS_STATUS_NOT_INITIALIZED when calling 'cublasCreate(handle)'
答：爆显存了，基本上跟CUDA有关的报错大都是爆显存……
```
```
9、报错：torch.multiprocessing.spawn.ProcessExitedException: process 0 terminated with exit code 3221225477
答：调大虚拟内存，管理员运行脚本
```
```
10、报错：'HParams' object has no attribute 'xxx'
答：无法找到音色，一般是配置文件和模型没对应，打开配置文件拉到最下面看看有没有你训练的音色
```
```
11、报错：The expand size of the tensor (768) must match the existing size (256) at non-singleton dimension 0.
